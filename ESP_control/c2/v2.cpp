...
// Add a new flag for serial connection
...
void controlTask(void *parameter) {
  ...
  while (1) {
    ...
    int pedalValue = analogRead(PEDAL_PIN);
    actual_brake = potValue;

    // computeSpeedPID();
    // if (pedalValue >= 1500) {
    //   // Map potValue to PWM range and apply to PEDAL_PIN
    //   int mappedValue = map(pedalValue, 880, 2680, 0, 255);
    //   ledcWrite(3, mappedValue);  // Assuming channel 3 is set up for PEDAL_PIN
    //   speed_u = mappedValue;  // Update speed_u for consistency in serial output
    // } else {
    //   // Use original PID control logic
...
void setup() {
  ...
  // Initialize desired values to prevent initial spike
  desired_velocity = 0;
  desired_brake = 0;
