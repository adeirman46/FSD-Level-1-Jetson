...
class CANSerialHandler:
    ...
    def __init__(self):
        self.running = True
        self.speed = 0
        self.mass = 1500  # Example mass of the vehicle in kg
    ...
    def velocity_control(self):
        ...
        # Determine desired velocity based on obstacle distance
        if self.obstacle_distance > safe_distance:
            return max_velocity
    ...
    def can_serial_thread(self):
        while self.running:
            ...
            brake = self.brake_control(velocity)
...
class LoggingThread(threading.Thread):
    def __init__(self, can_serial_handler):
        super().__init__()
        self.can_serial_handler = can_serial_handler
        self.running = True
    ...
    def run(self):
        while self.running:
            ...
            with open(log_filename, 'a') as log_file:
                log_entry = (
                    f"{current_time}, "
                    f"ACTUAL VELOCITY = {self.can_serial_handler.speed:0.0f}, "
                    f"DESIRED VELOCITY = {self.can_serial_handler.velocity_control()}, "
                    f"DESIRED BRAKE = {self.can_serial_handler.brake_control()}, "
                    f"OBSTACLE DISTANCE = {self.can_serial_handler.obstacle_distance:.2f}\n"
                )
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='v4s.pt', help='Path to the YOLO model.')
...
if __name__ == "__main__":
    ...
    logging_thread = LoggingThread(can_serial_handler)
