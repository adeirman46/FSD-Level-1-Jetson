...
class ZEDHandler:
    ...
    def process_frame(self, frame):
        ...
        # Overlay segmentation masks and calculate depth for segmentation
        if masks is not None:
            for mask in masks:
                colored_mask = np.zeros_like(frame)
                mask_indices = np.where(mask.masks[0])  # Get indices where mask is true
                colored_mask[mask_indices] = (0, 255, 0)  # Color the mask in green

                # Calculate the average depth for the mask area
                ...
                if len(valid_depths) > 0:
                    ...
                    mask_label_position = (mask_indices[1][0], mask_indices[0][0])  # Use the top-left corner of the mask
